...
static unsigned short readReg16(unsigned char ucAddr)
{
	...
	I2C_SoftStart();
	I2C_SoftWriteByte(I2C_SLAVE);
	I2C_SoftWriteByte(ucAddr);
	I2C_SoftStart();
	I2C_SoftWriteByte(I2C_SLAVE+1);
	tmp1=I2C_SoftReadByte(1) & 0xFF;
	tmp2=I2C_SoftReadByte(0) & 0xFF;
	I2C_SoftStop();
...
static void readMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
	...
	for(i=0;i<(iCount-1);i++) {
		*(pBuf++)=I2C_SoftReadByte(1) & 0xFF;
	}
	*pBuf=I2C_SoftReadByte(0) & 0xFF;
	I2C_SoftStop();
} 
...
static void writeReg16(unsigned char ucAddr, unsigned short usValue)
{
	...
	I2C_SoftWriteByte(I2C_SLAVE);
	I2C_SoftWriteByte(ucAddr);
	I2C_SoftWriteByte((unsigned char)(usValue >> 8));
	I2C_SoftWriteByte((unsigned char)usValue);
	I2C_SoftStop();
} 
...
// set interrupt configuration to "new sample ready"
  ...
  if (!performSingleRefCalibration(0x40)) { isDistanceSensorPresent=0; return 0; }
  writeReg(SYSTEM_SEQUENCE_CONFIG, 0x02);
  if (!performSingleRefCalibration(0x00)) { isDistanceSensorPresent=0; return 0; }
  writeReg(SYSTEM_SEQUENCE_CONFIG, 0xe8);
  return 1;
} 
...
LcdDrawRectangle(0,160,0,30,red);		// to do
LCD_SetFont(Arial_15x17,white);
LCD_ShowString(10,13,"RESULT_INTERRUPT"); 
LCD_ShowString(10,33,"timeout"); 
LCD_Refresh();	
while(1);	
...
LcdDrawRectangle(0,160,0,30,red);		// to do
LCD_SetFont(Arial_15x17,white);
LCD_ShowString(10,13,"SYSRANGE_START"); 
LCD_ShowString(10,33,"timeout"); 
LCD_Refresh();	
while(1);	
...
void tofGetModel(int *model, int *revision)
{
